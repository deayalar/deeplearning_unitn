...
class ValidationSplitter:
    ...
    def split(self, train_size:float, split_identities: bool=False,random_seed:int=None):
        ...
        all_ids = self._get_ids_from_images(images_list)
        ...
        print(f"Identities in train set size: {len(set(self._get_ids_from_images(train_set)))}")
        print(f"Identities in val set size: {len(set(self._get_ids_from_images(val_set)))}")
